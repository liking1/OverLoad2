...
{
	return size < str.size ? true : false;
}
bool String::operator>(const String& str) const
{
	return size > str.size ? true : false;
...
String String::combinateStrings(const String& str)
{
	String result;
...
{
	String result;
	result.setStringSize(strlen(this->mstring) + strlen(str.mstring) + 1);
	...
	{
		result.mstring[i] = str.mstring[j];
		j++;
	}
	return result;
}

void String::swapStrings(String&& one, String&& other) noexcept
{
